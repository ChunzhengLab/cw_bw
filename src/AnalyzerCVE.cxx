        ...
        for (int j = 0; j < 4; ++j) {
            ...
            for (int k = 0; k < 4; ++k) {
                p_delta_pair_etaGap[i][j][k] = new TProfile(
                    ...
                    Form("#delta %s vs #Delta#eta bin %d; #Delta#eta; #delta", tag.c_str(), k),
                    50, 0, 2);
                p_gamma_pair_etaGap[i][j][k] = new TProfile(
                    Form("p_gamma_%s_etaGap_%d", tag.c_str(), k),
                    Form("#gamma %s vs #Delta#eta bin %d; #Delta#eta; #gamma", tag.c_str(), k),
                    50, 0, 2);
            }
            // 3 sumPt bins
            for (int k = 0; k < 3; ++k) {
                p_delta_pair_sumPt[i][j][k] = new TProfile(
                    ...
                    Form("#delta %s vs sum p_{T} bin %d; sum p_{T}; #delta", tag.c_str(), k),
                    50, 0, 20);
                p_gamma_pair_sumPt[i][j][k] = new TProfile(
                    Form("p_gamma_%s_sumPt_%d", tag.c_str(), k),
                    Form("#gamma %s vs sum p_{T} bin %d; sum p_{T}; #gamma", tag.c_str(), k),
                    50, 0, 20);
            }
            // this-cent differentials
            p_delta_pair_sumPt_thisCent[i][j] = new TProfile(
                Form("p_delta_%s_sumPt_thisCent", tag.c_str()),
                Form("#delta %s vs sum p_{T} (this cent); sum p_{T}; #delta", tag.c_str()),
                50, 0, 20);
            p_gamma_pair_sumPt_thisCent[i][j] = new TProfile(
                Form("p_gamma_%s_sumPt_thisCent", tag.c_str()),
                Form("#gamma %s vs sum p_{T} (this cent); sum p_{T}; #gamma", tag.c_str()),
                50, 0, 20);
            p_delta_pair_etaGap_thisCent[i][j] = new TProfile(
                Form("p_delta_%s_etaGap_thisCent", tag.c_str()),
                Form("#delta %s vs #Delta#eta (this cent); #Delta#eta; #delta", tag.c_str()),
                50, 0, 2);
            p_gamma_pair_etaGap_thisCent[i][j] = new TProfile(
                Form("p_gamma_%s_etaGap_thisCent", tag.c_str()),
                Form("#gamma %s vs #Delta#eta (this cent); #Delta#eta; #gamma", tag.c_str()),
                50, 0, 2);
        }
    }

    // Pair count histograms
    h_pair_tot_etaGap = new TH1D("h_pair_tot_etaGap", "Total pairs vs #Delta#eta; #Delta#eta; Counts", 50, 0, 2);
    h_pair_tot_sumPt = new TH1D("h_pair_tot_sumPt", "Total pairs vs sum p_{T}; sum p_{T}; Counts", 50, 0, 20);
    h_pair_lbc_etaGap = new TH1D("h_pair_lbc_etaGap", "LBC pairs vs #Delta#eta; #Delta#eta; Counts", 50, 0, 2);
    h_pair_lbc_sumPt = new TH1D("h_pair_lbc_sumPt", "LBC pairs vs sum p_{T}; sum p_{T}; Counts", 50, 0, 20);
    ...
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            ...
            p_gamma_pair[i][j] = new TProfile(
                ...
                Form("#gamma %s; cent; #gamma", tag.c_str()),
                20, 0, 100);
            p_delta_pair[i][j] = new TProfile(
                Form("p_delta_%s", tag.c_str()),
                Form("#delta %s; cent; #delta", tag.c_str()),
                20, 0, 100);
        }
    }
}
...
void AnalyzerCVE::Process(const Event& evt) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if ((pid1!=2212 && pid1!=3122) || pt1<0.2f || pt1>5.0f || std::fabs(eta1)>0.8f) continue;

        for (size_t j = i+1; j < n; ++j) {
            const auto& p2 = particles[j];
            int pid2 = p2.PID();
            float pt2 = p2.Pt(), eta2 = p2.Eta(), phi2 = p2.Phi();
            if ((pid2!=2212 && pid2!=3122) || pt2<0.2f || pt2>5.0f || std::fabs(eta2)>0.8f) continue;
            ...
            // LBC pairs
            if (p1.GetSerialNumberLBCFriend()>=0 && p2.GetSerialNumberLBCFriend()>=0) {
