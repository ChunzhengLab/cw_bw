...
EventFactory::EventFactory(const Config &cfg)
  ...
  for (int i = 0; i < nBins; ++i) {
    ...
    auto fp = std::make_unique<TF1>(Form("fE_p_%d", i), MaxwellJuttnerKernel,
                                    mass_p, Emax_p, 2);
    ...
    auto fL = std::make_unique<TF1>(Form("fE_L_%d", i), MaxwellJuttnerKernel,
                                    mass_L, Emax_L, 2);
...
void EventFactory::BuildParticles(Event &evt) {
  ...
  for (unsigned int i = 0; i < multiplicity; ++i) {
    ...
    // 4) 质量/温度
    double mass =
        (std::abs(pid) == 2212 ? Particle::kMassProton : Particle::kMassLambda);
...
// 计算流场 boost 向量
TVector3 EventFactory::GetLocalBoostVector(float x, float y, int bin,
                                           int pid) const {
  ...
  TLorentzVector u;
  u.SetXYZT(
      std::sinh(rhob) * std::cos(phi_b), std::sinh(rhob) * std::sin(phi_b),
      std::cosh(rhob) * std::sinh(etas), std::cosh(rhob) * std::cosh(etas));
