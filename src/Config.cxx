...
Config Config::Load(const std::string& filename) {
    ...
    if (!config["k5TeV"]) throw std::runtime_error("Config::Load(" + filename + "): Missing required node: k5TeV");
    ...
    if (!config["ratioProtonLambda"]) throw std::runtime_error("Config::Load(" + filename + "): Missing required node: ratioProtonLambda");
