...
void integrateSpectra(const char* filename = "spec_v2.root")
{
  ...
  for (int i = 0; i < nC; ++i) {
    ...
    if (i == 2) yield1020 = yield[i];   // 10 - 20 %
    if (i == 3) yield2040 = yield[i];   // 20 - 40 %
    cout << std::left  << std::setw(20) << gname[i]
  ...
  for (int j = 0; j < nPred; ++j) {
    ...
    predGraphs.back()->SetNameTitle(targetName[j], targetName[j]);  // use desired names
  }
  ...
  for (int ipt = 0; ipt < nPts; ++ipt) {
    ...
    for (int j = 0; j < nPred; ++j) {
      ...
      predGraphs[j]->SetPointError(ipt,
                                   exLowArr[ipt],
                                   exHighArr[ipt],
                                   errY, errY);
      cout<<errY<<"    "<<endl;
  ...
  Color_t fitColors[] = {kRed, kOrange+7, kGreen+2, kBlue, kViolet,
                         kMagenta, kGray+1};
