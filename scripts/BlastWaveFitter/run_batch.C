...
void run(const char *centCode = "2030") {
  ...
  auto fetch = [](TFile *f, const std::string &name) -> TGraphAsymmErrors * {
    return f ? dynamic_cast<TGraphAsymmErrors *>(f->Get(name.c_str()))
             : nullptr;
  };
  ...
  if (!fMain || fMain->IsZombie())
    std::cerr << "Warning: cannot open spec_v2.root\n";
  ...
  if (!gSpec_p || !gSpec_L || !gV2_p || !gV2_L) {
    ...
    std::cout
        << "[run] Falling back to spec_lambda_infered.root for missing graphs."
        << std::endl;
    if (!fBak || fBak->IsZombie())
      std::cerr << "Warning: cannot open spec_lambda_infered.root\n";

    if (!gSpec_p)
      gSpec_p = fetch(fBak, gSpec_p_name);
    if (!gSpec_L)
      gSpec_L = fetch(fBak, gSpec_L_name);
    if (!gV2_p)
      gV2_p = fetch(fBak, gV2_p_name);
    if (!gV2_L)
      gV2_L = fetch(fBak, gV2_L_name);
  }
  ...
  // ---- 打印最终载入结果 ----
  std::cout << "=== Graph load summary for centrality " << code
            << " ===" << std::endl;
  auto report = [&](const std::string &name, TGraphAsymmErrors *g) {
    std::cout << "  " << std::left << std::setw(24) << name
              << (g ? "LOADED" : "NOT FOUND") << std::endl;
  };
...
void run_batch() {
  ...
  for (size_t i = 0; i < res.size(); ++i) {
    const auto &r = res[i];
    fout << centName[i] << ',' << (r.valid ? 1 : 0) << ',' << r.chi2 << ','
         << r.ndf;
    for (int p = 0; p < 8; ++p)
      fout << ',' << r.params[p];
    fout << '\n';
