...
void SetDataGraphs(TGraphAsymmErrors* spec_p,
                   TGraphAsymmErrors* spec_L,
                   TGraphAsymmErrors* v2_p,
...
public:
  ...
  double operator()(const std::vector<double>& par) const override {
    ...
    for (int i=0; i<gSpec_p->GetN(); ++i) {
      double x, y; gSpec_p->GetPoint(i, x, y);
    ...
    for (int i=0; i<gSpec_L->GetN(); ++i) {
      double x, y; gSpec_L->GetPoint(i, x, y);
    ...
    for (int i=0; i<gV2_p->GetN(); ++i) {
      double x, y; gV2_p->GetPoint(i, x, y);
    ...
    for (int i=0; i<gV2_L->GetN(); ++i) {
      double x, y; gV2_L->GetPoint(i, x, y);
...
void BlastWaveFitter() {
  ...
  // Proton spectrum
  TF1 *fSpec_p_fit = new TF1("fSpec_p_fit", [rho2_p_fit, Rx_fit](double *x, double *p){
    // p[0]=A_p, p[1]=betaT, p[2]=Tkin, p[3]=n_flow
    return p[0] * BW_Spectrum(x[0],
                              M_PROTON,
                              p[2],        // Tkin
                              p[1],        // betaT
                              p[3],        // n_flow
  ...
  fSpec_p_fit->SetParameters(result.UserState().Value(5),
                             result.UserState().Value(0),
                             result.UserState().Value(1),
  ...
  // Lambda spectrum
  TF1 *fSpec_L_fit = new TF1("fSpec_L_fit", [rho2_L_fit, Rx_fit](double *x, double *p){
  ...
  fSpec_L_fit->SetParameters(result.UserState().Value(6),
                             result.UserState().Value(0),
                             result.UserState().Value(1),
                             result.UserState().Value(2));
  ...
  // Proton v2
  TF1 *fV2_p_fit = new TF1("fV2_p_fit", [Rx_fit](double *x, double *p){
    // p[0]=betaT, p[1]=Tkin, p[2]=n_flow, p[3]=rho2_p
    return BW_v2shape(x[0],
                      M_PROTON,
                      p[1],   // Tkin
                      p[0],   // betaT
                      p[2],   // n_flow
  ...
  fV2_p_fit->SetParameters(result.UserState().Value(0),
                           result.UserState().Value(1),
                           result.UserState().Value(2),
  ...
  // Lambda v2
  TF1 *fV2_L_fit = new TF1("fV2_L_fit", [Rx_fit](double *x, double *p){
  ...
  fV2_L_fit->SetParameters(result.UserState().Value(0),
                           result.UserState().Value(1),
                           result.UserState().Value(2),
